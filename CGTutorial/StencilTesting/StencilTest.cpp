...
int main()
{
    ...
    HAUtilities::Model stencilModel(stencilVertices, sizeof(stencilVertices) / sizeof(float), false, true, cubeTextures);
    HAUtilities::Shader shader("Stencil.vs", "Stencil.fs");
    HAUtilities::Shader shaderSingleColor("Stencil.vs", "Stencil_SingleColor.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shader.SetUniform("viewPos", camPos.x, camPos.y, camPos.z);
        shader.SetMatrixUniform("viewMat", 1, GL_FALSE, glm::value_ptr(freeMovementCamera.GetViewMatrix()));
        shader.SetMatrixUniform("projectionMat", 1, GL_FALSE, glm::value_ptr(freeMovementCamera.GetProjectionMatrix()));
        ...
        shader.SetMatrixUniform("modelMat", 1, GL_FALSE, glm::value_ptr(modelMat));
        ...
        //stencilModel.Draw(shader);
        cubeModel.Draw(shader); //draw and set stencil to 1 
        ////draw scaled up container
        //glStencilMask(0x00);
        glDisable(GL_DEPTH_TEST);
        shaderSingleColor.Use();
        shaderSingleColor.SetUniform("viewPos", camPos.x, camPos.y, camPos.z);
        shaderSingleColor.SetMatrixUniform("viewMat", 1, GL_FALSE, glm::value_ptr(freeMovementCamera.GetViewMatrix()));
        shaderSingleColor.SetMatrixUniform("projectionMat", 1, GL_FALSE, glm::value_ptr(freeMovementCamera.GetProjectionMatrix()));
        modelMat = glm::mat4(1.0f);
        modelMat = glm::translate(modelMat, glm::vec3(0.0f, 0.01f, 0.0f));
        modelMat = glm::scale(modelMat, glm::vec3(1.05f, 1.05f, 1.05f));
        shaderSingleColor.SetMatrixUniform("modelMat", 1, GL_FALSE, glm::value_ptr(modelMat));
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        glStencilOp(GL_KEEP, GL_REPLACE, GL_REPLACE);
        cubeModel.Draw(shaderSingleColor); //draw scaled up container using single color when stencil not equal 1
